...
class EmailPromptService:
    def __init__(self, app, db, email_log, study, user):
        self.app = app
        self.db = db
    ...
    def send_prompts(self, recipients, send_method, log_type):
        for rec in recipients:
            email_logs = self.db.session.query(self.email_log)\
                .filter_by(user_id=rec.id)\
                .filter_by(type=log_type) \
                .order_by(self.email_log.last_updated).all()
    ...
    def send_prompting_email(self, user, send_method, log_type):
        current_studies = self.db.session.query(self.study).filter_by(status='currently_enrolling').all()
        for study in current_studies:
            study.link = self.app.config['SITE_URL'] + '/#/study/' + str(study.id)
        tracking_code = send_method(user, current_studies)
        log = self.email_log(user_id=user.id, type=log_type, tracking_code=tracking_code)
        self.db.session.add(log)
        self.db.session.commit()

    def send_confirm_prompting_emails(self):
        recipients = self.db.session.query(self.user).filter_by(password=None).all()
        self.send_prompts(recipients, EmailService(self.app).async_confirm_email, 'confirm_email')

    def send_complete_registration_prompting_emails(self):
        confirmed_users = self.db.session.query(self.user).filter(self.user.password is not None).all()
        recipients = [u for u in confirmed_users if u.self_registration_complete() is False]
        self.send_prompts(recipients, EmailService(self.app).complete_registration_prompt_email,
        ...
        for u in confirmed_users:
            if (u.get_self_participant() is not None) \
                ...
                if (len(dependents) == 0) or (len(incomplete_dependents) > 0):
                    recipients.append(u)
        self.send_prompts(recipients, EmailService(self.app).complete_dependent_profile_prompt_email,

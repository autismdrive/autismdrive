...
class TestImportCase(BaseTestQuestionnaire):
    ...
    def setUp(self):
        super().setUp()
        settings.MASTER_URL = "http://na.edu"
        settings.MASTER_EMAIL = "dan@test.com"
        settings.MASTER_PASS = "12345"

    @httpretty.activate
    def test_httppretty(self):
        # Just wanted a simple exmple to show how to use HTTP Pretty
        httpretty.register_uri(httpretty.GET, "https://httpbin.org/ip", body='{"origin": "127.0.0.1"}')
    ...
    def test_login(self):
        # Just wanted a simple exmple to show how to use HTTP Pretty
        httpretty.register_uri(httpretty.POST, "http://na.edu/api/login_password", body='{"token": "my_token"}')
    ...
    def test_headers(self):
        ...
        httpretty.register_uri(
            httpretty.GET, "http://na.edu/api/session", body='{"error": "not logged in"}', status=400
        )
        httpretty.register_uri(
            httpretty.POST,
    ...
    def get_data_importer_setup_auth(self):
        httpretty.register_uri(httpretty.GET, "http://na.edu/api/session", body='{"email": "dan@test.com"}', status=200)
        data_importer = ImportService()
    ...
    def test_import_calls_delete_on_sensitive_data(self):
        ...
        httpretty.register_uri(
            httpretty.GET, "http://na.edu/api/export/clinical_diagnoses_questionnaire", body=json_q, status=200
        )
        expected_delete_url = "http://na.edu/api/q/clinical_diagnoses_questionnaire/" + str(id)
        httpretty.register_uri(httpretty.DELETE, expected_delete_url, body=json_q, status=200)
    ...
    def test_import_does_not_call_delete_on_non_sensitive_data(self):
        ...
        httpretty.register_uri(
            httpretty.GET, "http://na.edu/api/export/employment_questionnaire", body=json_q, status=200
        )
        data_importer = self.get_data_importer_setup_auth()
        date = datetime.datetime.now()
        data = data_importer.request_data(export_list)
